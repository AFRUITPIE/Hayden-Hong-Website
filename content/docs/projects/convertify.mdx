...
<Steps>
  <Step>
    <h3>Get playlist tracks from source</h3>

    For the playlist source, Convertify made an API call to retrieve the tracks.
    Each track from Apple Music or Spotify comes with a unique ID, and those IDs
    don't match between services.
  </Step>
  <Step>
    <h3>Search for similar tracks in destination</h3>

    To figure out which track ID to add to the destination playlist, Convertify
    called the destination's search API. A search request similar to this was
    made:
    ...
    After repeating this for every track in the source playlist, we had the
    track IDs we wanted to add to the destination playlist.
  </Step>
  <Step>
    <h3>Populate destination with tracks</h3>

    Once all on-destination IDs were collected, Convertify would call the
    destination's API (batched when supported) to append the tracks.
  </Step>
</Steps>
